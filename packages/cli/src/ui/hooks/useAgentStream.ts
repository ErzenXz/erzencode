...
import { createAIAgent } from "@erzencode/core/ai-agent";
import { resolveThinkingConfig, getApiKeyAsync } from "@erzencode/core/config";
import { getModel } from "@erzencode/core/models";
  ...
  const submitMessage = useCallback(
    async (
        ...
        for await (const event of agent.stream(messagesForApi as any)) {
          ...
          if (event.type === "reasoning") {
            ...
            const isLoadingResult =
              typeof resultOutputRaw === "object" &&
              resultOutputRaw !== null &&
              ["loading", "waiting_for_input"].includes(String((resultOutputRaw as any).status));

            if (isPreliminary && isLoadingResult) {
      ...
      if (!abortControllerRef.current?.signal.aborted) {
        ...
        if (shouldCompact(latestPromptTokens, contextLimit, 0.85)) {
