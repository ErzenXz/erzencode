...
import type { AgentConfig as CodingAgentConfig, AgentMode as CodingAgentMode } from "@erzencode/core/ai-agent";
import {
...
import { getAllThemes, getCurrentTheme, initTheme, setTheme } from "@erzencode/core/themes";
import {
...
import { modelSupports, preloadDynamicModels, getModel } from "@erzencode/core/models";
import {
  compactConversation,
  formatCompactionResult,
  createCompactedMessage,
  estimateConversationTokens,
} from "@erzencode/core/compaction";
import {
  CodebaseIndexer,
  wasPromptShown,
  markPromptShown,
  ...
  type IndexStats,
  type SearchResult,
} from "@erzencode/core/indexer";
import figures from "figures";

import {
  type Stage, type SlashCommand,
  type ChatMessage, type ToolPart,
...
function clearTerminal(): void {
  // Clear screen only (avoid wiping scrollback)
  // 2J: clear screen, H: cursor home, 0m: reset styles
...
export const App: React.FC<AppProps> = ({
  ...
  useEffect(() => {
    setTodoUpdateCallback((next: TodoItem[]) => setTodos(next));
  ...
  const messageItems: SelectorItem[] = useMemo(() => {
    const userMessages = sessionState.currentSession.messages.filter((m) => m.role === "user");
    return userMessages.map((m, idx) => {
      const time = new Date(m.timestamp).toLocaleTimeString();
      const preview = truncate(String(m.content ?? ""), Math.max(20, terminalWidth - 20));
      return {
        id: m.id,
        label: `${idx + 1}. ${time} - ${preview || "(empty message)"}`,
        status: "none",
      };
    });
  }, [sessionState.currentSession.messages, terminalWidth]);

  const rewindItems: SelectorItem[] = useMemo(() => {
    return sessionState.currentSession.messages.map((m, idx) => {
      const time = new Date(m.timestamp).toLocaleTimeString();
      const role =
        m.role === "assistant"
          ? "assistant"
      ...
      return {
        id: m.id,
        label: `${idx + 1}. ${role} ${time} - ${preview || "(empty message)"}`,
        status: "none",
      };
    });
  }, [sessionState.currentSession.messages, terminalWidth]);
  ...
  const getModalItemCount = useCallback(() => {
    ...
    switch (modalState.activeModal) {
      ...
      case "messages": {
        if (!modalSearchQuery.trim()) return messageItems.length;
        const query = modalSearchQuery.toLowerCase();
      ...
      case "rewind": {
        if (!modalSearchQuery.trim()) return rewindItems.length;
        const query = modalSearchQuery.toLowerCase();
        return rewindItems.filter(
          (item) =>
            item.label.toLowerCase().includes(query) ||
            item.id.toLowerCase().includes(query)
        ).length;
      ...
      case "provider": {
        ...
        return PROVIDERS.filter(
          (p: ProviderInfo) => p.name.toLowerCase().includes(query) || p.id.toLowerCase().includes(query)
        ).length;
      }
    ...
    agentConfig.availableModels.length,
    sessionState.sessions.length,
    sessionState.currentSession.messages.length,
    messageItems.length,
    rewindItems.length,
    modalSearchQuery,
  ]);

  const reversePatch = useCallback((patch: string): string => {
    return patch
      .split("\n")
      .map((line) => {
        ...
        if (line.startsWith("@@")) {
          return line.replace(
            /@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@/,
            (_match, oldStart, oldLines, newStart, newLines) => {
              const left = `-${newStart}${newLines ? `,${newLines}` : ""}`;
              const right = `+${oldStart}${oldLines ? `,${oldLines}` : ""}`;
              return `@@ ${left} ${right} @@`;
            }
          );
        }
  ...
  const rewindSessionEdits = useCallback(async () => {
    const messages = sessionState.currentSession.messages;
    const changes: Array<{ path: string; patch: string; tool: string }> = [];
    ...
    for (let i = messages.length - 1; i >= 0; i -= 1) {
      ...
      for (let j = msg.parts.length - 1; j >= 0; j -= 1) {
        ...
        if (!["edit", "edit_file", "write", "write_file"].includes(tool.name)) continue;
        ...
        const resolvedPath = path.isAbsolute(String(rawPath))
          ? String(rawPath)
          : path.resolve(workingDirectory, String(rawPath));
        changes.push({ path: resolvedPath, patch, tool: tool.name });
      }
    }
    ...
    if (changes.length === 0) {
      sessionState.addMessage({
        id: generateId(),
        role: "system",
        content: "No reversible file changes found in this session.",
        timestamp: Date.now(),
      });
      return;
    }
    ...
    for (const change of changes) {
      let currentContent = "";
      try {
        currentContent = await fs.promises.readFile(change.path, "utf-8");
      } catch {
        skipped.push({ path: change.path, reason: "file missing" });
        continue;
      }

      const reversed = reversePatch(change.patch);
      const updated = applyPatch(currentContent, reversed, { fuzzFactor: 2 });
      if (updated === false) {
        skipped.push({ path: change.path, reason: "patch did not apply cleanly" });
        continue;
      }
      try {
        await fs.promises.writeFile(change.path, updated, "utf-8");
        applied.push(change.path);
      } catch {
        skipped.push({ path: change.path, reason: "failed to write file" });
      }
    }
  ...
  }, [sessionState, workingDirectory, reversePatch]);
  ...
  const handleCommand = useCallback((commandLine: string) => {
    ...
    switch (cmd.name) {
      ...
      case "theme":
        ...
        modalState.setSelectionIndex(Math.max(0, getAllThemes().findIndex((t: { id: string }) => t.id === themeId)));
        break;
      case "web":
        (async () => {
          try {
      ...
      case "sessions":
        ...
        break;
      case "messages":
        modalState.openModal("messages");
        modalState.setSelectionIndex(0);
        setModalSearchQuery("");
        break;
      case "settings": modalState.openModal("settings"); break;
      case "provider":
        modalState.openModal("provider");
        modalState.setSelectionIndex(PROVIDERS.findIndex((p: ProviderInfo) => p.id === agentConfig.provider));
        break;
      case "thinking":
      ...
      case "rewind":
        if (agentStream.isThinking) {
          agentStream.setStatus("Stop the current task before rewinding");
          break;
        }
        if (sessionState.currentSession.messages.length === 0) {
          agentStream.setStatus("No messages to rewind");
          break;
        }
        modalState.openModal("rewind");
        modalState.setSelectionIndex(sessionState.currentSession.messages.length - 1);
        setModalSearchQuery("");
        break;
      case "cost": {
        ...
        (async () => {
          ...
          try {
            if (sub === "status") {
              ...
              const lines = pending.slice(0, 3).map((p: { id: string; command: string }) => `${p.id}: ${p.command.slice(0, 60)}`).join(" | ");
  ...
  const jumpToMessage = useCallback((messageId: string) => {
    const completed = sessionState.currentSession.messages.filter((m) => !m.isStreaming);
    const targetIndex = completed.findIndex((m) => m.id === messageId);
    if (targetIndex < 0) {
      agentStream.setStatus("Message not found");
      return;
    }
    ...
    const windowSize = 50;
    const end = Math.min(completed.length, targetIndex + 1);
    const scroll = completed.length - end;
    setScrollOffset(Math.max(0, scroll));
    agentStream.setStatus(`Jumped to message ${targetIndex + 1} of ${completed.length}`);
  }, [sessionState.currentSession.messages, streamingMessage, agentStream]);
    ...
    switch (modalState.activeModal) {
      ...
      case "rewind": {
        ...
        if (selected) {
          ...
          const targetIndex = allMessages.findIndex((m) => m.id === selected.id);
          if (targetIndex >= 0) {
            const retained = allMessages.slice(0, targetIndex + 1);
            sessionState.updateSession(sessionState.currentSessionId, {
              messages: retained,
            });
            const hasCutoff =
              contextCutoffMessageId &&
              retained.some((m) => m.id === contextCutoffMessageId);
            if (!hasCutoff) {
              setContextSummary(null);
              setContextCutoffMessageId(null);
            }
            ...
            setFrozenMessages(null);
            setScrollOffset(0);
            agentStream.setStatus(
              `Rewound to message ${targetIndex + 1} of ${allMessages.length}`
            );
          }
        }
        modalState.closeModal();
        ...
        if (selectedProvider) {
          (async () => {
            ...
            if (apiKey || selectedProvider.id === "ollama") {
              ...
              setModalSearchQuery("");
              try {
                const { startDeviceFlow, pollForToken } = await import("@erzencode/core/copilot-auth");
  ...
  useInput((inputKey, key) => {
    ...
    switch (action.type) {
          ...
          (async () => {
            try {
              ...
              const indexer = new CodebaseIndexer({
                projectPath: workingDirectory,
                voyageApiKey: voyageKey,
                onProgress: (p: IndexingProgress) => setIndexProgress(p),
              });
      ...
      case "input":
        ...
        } else if (["models", "provider", "theme", "sessions", "thinking", "messages"].includes(modalState.activeModal)) {
  ...
  const liveContextTokens = useMemo(() => {
    ...
    if (streamingMessage?.isStreaming && streamingMessage.content) {
      promptMessages.push({
        role: "assistant",
        content: String(streamingMessage.content ?? ""),
      });
    }
    return estimateConversationTokens(promptMessages);
  }, [
    sessionState.currentSession.messages,
    contextCutoffMessageId,
    contextSummary,
  ...
  const contextLeft = Math.max(0, contextLimit - liveContextTokens);
  const contextLeftPct =
    contextLimit > 0 ? Math.max(0, Math.round((contextLeft / contextLimit) * 100)) : 0;
  ...
  const renderInlineSelector = () => {
    ...
    if (modalState.activeModal === "messages") {
      return (
        <InlineSelector
          title="Messages"
          items={messageItems}
    ...
    if (modalState.activeModal === "rewind") {
      return (
        <InlineSelector
    ...
    <Box flexDirection="column" width={terminalWidth}>
      ...
      >
        <ChatFeed
          ...
          onMaxScrollOffsetChange={setMaxScrollOffset}
      ...
      <Box flexDirection="column" flexShrink={0}>
          ...
          <Box gap={2}>
            ...
            {scrollOffset > 0 && (
              <Text color={themeColors.warning}>
                scroll {scrollOffset}/{Math.max(1, maxScrollOffset)}
              </Text>
            )}
            {expandedTools && (
        ...
        <InputBox
          ...
          themeColors={themeColors}
          hideComposer={["models", "provider", "theme", "sessions", "thinking", "messages", "rewind"].includes(modalState.activeModal)}
        />
      </Box>
    </Box>
  );
