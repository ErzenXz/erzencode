...
import { createAIAgent, type AgentConfig } from "@erzencode/core/ai-agent";
...
program
  ...
  .action(async (providerArg?: string) => {
    ...
    const providerName = PROVIDERS.find((p: ProviderInfo) => p.id === provider)?.name ?? provider;
...
program
  ...
  .action(async () => {
    ...
    const providerRows = PROVIDERS.map((p: ProviderInfo) => {
...
async function promptForProviderModel(
  ...
  const { selectedProvider } = await inquirer.prompt<{
  ...
  }>([
    {
      ...
      choices: PROVIDERS.map((p: ProviderInfo) => ({
